                ...
                */
                ...
                if ( cc == -1) {
                    ...
                    printf("%s: recvfrom error\n", argv[0]);
										h = ErrorMsg(NODEFINIDO, "recvfrom error");
									  sendto (s_UDP, h, 2+2+strlen("recvfrom error")+1,0, (struct sockaddr *)&clientaddr_in, addrlen);
                    exit (1);
                    }

								printMSG(buffer);

								if(getPacketType(buffer)==1) serverUDPRecibeFichero(s_UDP, getFilename(buffer), clientaddr_in);
...
void serverUDPEnviaFichero(int s, char * Nombrefichero, struct sockaddr_in clientaddr_in)
{
  ...
  if(fichero==NULL){
		sendErrorMSG(s, clientaddr_in, FICHERONOENCONTRADO, "No se ha encontrado el fichero");
		return;
	}
  ...
  if(datosFichero==NULL || UltimosdatosFichero==NULL){
		sendErrorMSG(s, clientaddr_in, NODEFINIDO, "Error al hacer el calloc");
		fclose (fichero);
  	return;
	}
  while(fin!=2){
    ...
    if(getPacketType(asentimiento)!=4){
		  sendErrorMSG(s, clientaddr_in, OPERACIONILEGAL, "Numero paquete incorrecto");
			fclose(fichero);
			free(datosFichero);
			free(UltimosdatosFichero);
			return;
		...
		if(getPacketNumber(parteFichero)!=n){
		  sendErrorMSG(s, clientaddr_in, OPERACIONILEGAL, "Numero asentimiento incorrecto");
			fclose(fichero);
			free(datosFichero);
			free(UltimosdatosFichero);
			return;
...
void serverUDPRecibeFichero(int s, char * Nombrefichero, struct sockaddr_in clientaddr_in)
{
  ...
  addrlen = sizeof(struct sockaddr_in);
	...
	fichero = fopen(rutaFichero,"r");
  if(fichero!=NULL){
		sendErrorMSG(s, clientaddr_in, FICHEROYANOEXISTE, "El fichero ya existe");
		fclose(fichero);
		return;
	}
